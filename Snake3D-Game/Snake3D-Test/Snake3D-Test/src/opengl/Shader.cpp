...
GLuint Shader::loadShader(std::string shaderFileName, GLuint shaderType) {
  ...
  if (!checkCompileStats(shaderFileName, shader)) {
    Logger::log(2, "%s error: compiling shader '%s' failed\n", __FUNCTION__, shaderFileName.c_str());
    return 0;
  }

  Logger::log(2, "%s: shader %#x loaded and compiled\n", __FUNCTION__, shader);
  return shader;
}
...
bool Shader::createShaderProgram(std::string vertexShaderFileName, std::string fragmentShaderFileName) {
  ...
  if (!checkLinkStats(vertexShaderFileName, fragmentShaderFileName, mShaderProgram)) {
    Logger::log(2, "%s error: program linking from vertex shader '%s' / fragment shader '%s' failed\n", __FUNCTION__, vertexShaderFileName.c_str(), fragmentShaderFileName.c_str());
  ...
  Logger::log(2, "%s: shader program %#x successfully compiled from vertex shader '%s' and fragment shader '%s'\n", __FUNCTION__, mShaderProgram, vertexShaderFileName.c_str(), fragmentShaderFileName.c_str());
  return true;
}
...
bool Shader::checkCompileStats(std::string shaderFileName, GLuint shader) {
  ...
  if (!isShaderCompiled) {
    ...
    Logger::log(2, "%s error: shader compile of shader '%s' failed\n", __FUNCTION__, shaderFileName.c_str());
    Logger::log(2, "%s compile log:\n%s\n", __FUNCTION__, shaderLog.data());
    return false;
  }
...
bool Shader::checkLinkStats(std::string vertexShaderFileName, std::string fragmentShaderFileName, GLuint shaderProgram) {
  ...
  if (!isProgramLinked) {
    ...
    Logger::log(2, "%s error: program linking of shaders '%s' and '%s' failed\n", __FUNCTION__, vertexShaderFileName.c_str(), fragmentShaderFileName.c_str());
...
std::string Shader::loadFileToString(std::string fileName) {
  ...
  if (inFile.is_open()) {
    ...
    Logger::log(2, "%s error: could not open file %s\n", __FUNCTION__, fileName.c_str());
    Logger::log(2, "%s error: system says '%s'\n", __FUNCTION__, strerror(errno));
    return std::string();
  }

  if (inFile.bad() || inFile.fail()) {
    Logger::log(2, "%s error: error while reading file %s\n", __FUNCTION__, fileName.c_str());
    inFile.close();
    return std::string();
  }

  inFile.close();
  Logger::log(2, "%s: file %s successfully read to string\n", __FUNCTION__, fileName.c_str());
  return str;
}
