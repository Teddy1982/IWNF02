...
bool Framebuffer::init(unsigned int width, unsigned int height) {
  ...
  Logger::log(2, "%s: added color buffer\n", __FUNCTION__);
  ...
  Logger::log(2, "%s: added depth renderbuffer\n", __FUNCTION__);
...
bool Framebuffer::checkComplete() {
  ...
  if (result != GL_FRAMEBUFFER_COMPLETE) {
    Logger::log(2, "%s error: framebuffer is NOT complete\n", __FUNCTION__);
    return false;
  }

  glBindFramebuffer(GL_FRAMEBUFFER, 0);
  Logger::log(2, "%s: framebuffer is complete\n", __FUNCTION__);
  return true;
}
